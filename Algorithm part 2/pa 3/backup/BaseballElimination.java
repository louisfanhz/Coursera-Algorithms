...
public class BaseballElimination {
    ...
    public BaseballElimination(String filename) {
        ...
        while (in.hasNextLine()) {
            String[] teamInfo = in.readLine().split("\\s+");
            teams.put(teamInfo[0], i);
    ...
    private Queue<String> getEliminators(String team) {
        ...
        for (String s : teams()) {
            if (!s.equals(team)) {
                ...
                edges.add(new FlowEdge(teams.get(s), tID, capacity));
            }
        }
        ...
        for (String s : teams()) {
            if (!s.equals(team) && FF.inCut(teams.get(s)))
                eliminator.set(teams.get(s), s);
        }
